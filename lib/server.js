...
class ODataProcessor extends stream_1.Transform {
    ...
    __read(ctrl, part, params, data, filter) {
        return new Promise((resolve, reject) => {
            ...
            let fn = odata_1.odata.findODataMethod(ctrl, method, part.key);
            if (!fn)
            ...
            switch (method) {
                ...
                case "patch":
                    ...
                    if (bodyParam)
                        params[bodyParam] = extend(this.body, data.foreignKeys);
                    if (!part.key) {
            ...
            return currentResult.then((result) => {
                if (isStream(result) && !edm_1.Edm.isMediaEntity(this.ctrl.prototype.elementType)) {
                    return new Promise((resolve, reject) => {
                ...
                try {
                    ...
                    if (boundOp == expOp) {
                        ...
                        if (edm_1.Edm.isMediaEntity(elementType)) {
                            ...
                            return resolve(result_1.ODataResult.Ok(expResult, typeof expResult == "object" ? "application/json" : "text/plain"));
                        }
                    }
