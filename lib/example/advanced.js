    ...
    find(filter) {
        if (filter) {
            ...
            return products.map((product) => {
                ...
                return product;
            }).filter((product) => filterFn(product)).map(product => new model_1.Product(product));
        }
        return products.map(product => new model_1.Product(product));
    }
    findOne(key) {
        let product = products.filter(product => product._id.toString() == key)[0];
        return product ? new model_1.Product(product) : null;
    }
};
__decorate([
...
let CategoriesController = class CategoriesController extends index_1.ODataController {
    find(query) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return yield (yield mongodb()).collection("Categories").find(mongodbQuery.query, mongodbQuery.projection, mongodbQuery.skip, mongodbQuery.limit).toArray().then((categories) => categories.map(category => new model_1.Category(category)));
        });
    }
    findOne(key, query) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            }).then((category) => new model_1.Category(category));
        });
    }
    GetFirstProduct() {
        return products[0];
