...
const expCalls = {
    ...
    $ref: function (processor) {
        ...
        let fn = odata_1.odata.findODataMethod(processor.ctrl, processor.method + "/" + prevPart.name + "/$ref", routePart.key || []);
        ...
        let linkPath = new visitor_1.ResourcePathVisitor().Visit(linkAst);
        let linkPart = linkPath.navigation[linkPath.navigation.length - 1];
        let ctrl = processor.ctrl;
        let params = {};
        if (linkPart.key)
            linkPart.key.forEach((key) => params[key.name] = key.value);
        let fnDesc = fn;
