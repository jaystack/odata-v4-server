...
function createMetadataJSON(server) {
    ...
    let resolveType = (elementType, parent) => {
        ...
        let typeDefinition = {
            ...
            openType: edm_1.Edm.isOpenType(elementType) || elementType === Object,
        ...
        Object.keys(operations).forEach((operation) => {
            ...
            if (edm_1.Edm.isFunction(operations[operation], operation)) {
                let type = edm_1.Edm.getReturnType(operations[operation], operation);
                if (typeof type == "function") {
                    operations[operation].namespace = namespace;
                    let definitionContainer = edm_1.Edm.isComplexType(operations[operation], operation) ? "complexType" : "entityType";
    ...
    propNames.forEach((i) => {
        if (i != "$metadata" && server.prototype[i]) {
            ...
            if (edm_1.Edm.isFunctionImport(server, i)) {
                let type = edm_1.Edm.getReturnType(server, i);
                if (typeof type == "function") {
                    let definitionContainer = edm_1.Edm.isComplexType(server, i) ? "complexType" : "entityType";
